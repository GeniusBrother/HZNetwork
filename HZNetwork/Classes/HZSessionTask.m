                      ...
                      delegate:(id<HZSessionTaskDelegate>)delegate taskIdentifier:(NSString *)taskIdentifier
{
    ...
    task.method = method;
    task.path = path;
    task.params = params.isNoEmpty?[NSMutableDictionary dictionaryWithDictionary:params]:nil;
                   ...
                   taskIdentifier:(NSString *)taskIdentifier
{
    ...
    task.pathValues = pathValues.isNoEmpty?[NSMutableArray arrayWithArray:pathValues]:nil;
    return task;
}
...
{
    BOOL invalid = !key.isNoEmpty || (value == nil);
    if (invalid) return;
...
- (BOOL)codeIsRight
{
    ...
    return self.responseObject && [[self.responseObject objectForKeyPath:codeKeyPath] integerValue] == [HZNetworkConfig sharedConfig].rightCode;
}
...
- (NSString *)absoluteURL
{
    ...
    if ([self.method caseInsensitiveCompare:@"GET"] == NSOrderedSame && self.params.isNoEmpty) {
        NSString *separator = [absoluteURL rangeOfString:@"?"].length == 0?@"?":@"&";
        absoluteURL = [absoluteURL stringByAppendingFormat:@"%@%@",separator,self.params.keyValueString]; //查询字符串格式
    }
